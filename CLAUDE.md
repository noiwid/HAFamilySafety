...
- **The frontend may not be there to finish the flow** (2.0.9, issue #50). The Home Assistant Android app opens the external step in an in-app browser (a plain Android WebView, UA `; wv)`, no "Home Assistant/"), **closes the setup dialog**, and ignores `window.close()`. Nobody then fetches the next step, so the sign-in was captured but never saved. After the browser callback, `_async_renotify_frontend()` first re-sends `data_entry_flow_progressed` every 3 s while the flow sits on a `_FRONTEND_WAIT_STEPS` step; if one of `_AUTO_ADVANCE_STEPS` is left alone for `_AUTO_ADVANCE_AFTER_SECONDS` (20 s) it calls `async_configure` itself, submitting the field-less `auth_success`/`reauth_success` forms. The grace period is what keeps desktop behaviour intact (configuring straight from the callback raced the frontend into `UnknownFlow` before). The completion page then sends the app back with `homeassistant://navigate/config/integrations/integration/microsoft_family_safety`, and has explicit light/dark colours (the dark WebView rendered black text on black).
- **Microsoft can insert interactive steps into the silent renewal.** Since 2026-09-24 the "Continue" form may post to `account.live.com/tou/accrue` (updated Terms of Use). `_note_account_interrupt()` records it as `account_interrupt = "terms_of_use"` (Connection sensor attribute, notification text) and it is **never submitted**: accepting terms is the user's decision. Accepting them on account.microsoft.com is enough; the next silent renewal then succeeds without a reauth.
...
- **A roster 404 is per member, not per device.** `FamilySafety.create()` and `update()` call five mobile endpoints per child (`deviceScreenTimeUsage`, `appUsage`, `devices`, `devicelimits/.../overrides`, `Spending`); `Family.UnableToFindTargetResource` / `RosterError` comes from one of them and the `MachineName` in the message is Microsoft's server, not the child's device. In 1.1.2 that exception aborts the whole roster, so `_pyfamilysafety_compat._patch_account_roster_tolerance()` (2.0.7, issue #42) replaces `Account.update`: the failing endpoint is swallowed, the member keeps empty shapes (`_empty_screentime_report()`, `{"appActivity": []}`, `[]`) so `Device.from_dict` and `_get_applications` do not crash, and `account.roster_errors` lists the endpoints; the coordinator surfaces them as `roster_errors` on each account, on the Connection sensor, and through the `familysafety_roster_error` notification. Only the roster request itself (`get_accounts`) still goes through the `UpdateFailed` branch in `_async_setup_api`. Any other error propagates unchanged.
